...
# Additional testing and evaluation metrics
model.eval()
with torch.no_grad():
    ...
    # Accuracy
    accuracy = accuracy_score(y_val_indices.cpu(), val_predictions.cpu())
    print(f"Final Validation Accuracy: {accuracy * 100:.2f}%")

    # Precision, Recall, F1-score
    precision = precision_score(y_val_indices.cpu(), val_predictions.cpu(), average='weighted')
    recall = recall_score(y_val_indices.cpu(), val_predictions.cpu(), average='weighted')
    f1 = f1_score(y_val_indices.cpu(), val_predictions.cpu(), average='weighted')
